...
function prep_table(){
   ...
   createtable(array_country_status, array_stove_type,array_country_status_fuels, array_fuel)

}

function createtable(country_status, stove_status, country_status_fuel, app_fuels) {
    ...
    for (let i = 0; i < stove_types.length; i++) {
        if (array_stove_type[i]) {
            ...
            var values = ["No Data Available", "Tier 0", "Tier 1", "Tier 2", "Tier 3", "Tier 4"];
    ...
    var tableBody = document.getElementById("fuel_info_table_body")
    tableBody.innerHTML = ""
    // Create rows for each stove type
    for (let i = 0; i < array_country_status_fuels.length; i++) {
    	if (array_country_status_fuels[i]) {
			// Creating a row if fuels are sold in a country
            var tr = document.createElement('TR');
            tableBody.appendChild(tr);
            var td1 = document.createElement('TD');
            td1.appendChild(document.createTextNode(country[i]));
            tr.appendChild(td1);
    	...
    	// Adding the applicable countries
            var td2 = document.createElement('TD');
            td2.appendChild(document.createElement('div'))
            for (let j = 0; j < array_fuels.length; j++) {
                if (app_fuels[j]) {
                    var checkbox = document.createElement('input');
                    checkbox.type = "checkbox";
                    checkbox.value = j.toString();
                    checkbox.id = country_code[i] + "_" + fuels_code[j];

                    var label = document.createElement('label')
                    label.classList.add('in_table');
                    var t_span = document.createElement('span');
                    t_span.innerHTML = array_fuels[j];
                    label.appendChild(checkbox);
                    label.appendChild(t_span)
                    td2.appendChild(label)
                }
	        ...
	        for (let j = 0; j < array_fuels.length; j++) {
                if (app_fuels[j]) {
			        var checkbox = document.createElement('input');
			        checkbox.id = "amount_sold_" + country_code[i]+"_"+ fuels_code[j];
			        checkbox.type = "number";
			        checkbox.placeholder = "Amount sold in kg/pound/litre"
			        var label = document.createElement('label')
			        label.classList.add('in_table');
			        var t_span = document.createElement('span');
			        label.appendChild(checkbox);
    ...
    for (let i = 0; i < array_fuel.length; i++) {
        ...
        td2.appendChild(document.createElement('div'))
        var checkbox = document.createElement('input');
        checkbox.id = "amount_sold_" + fuels_code[i];
        checkbox.type = "number";
        checkbox.placeholder = "Amount sold in kg/pound/litre"
        var label = document.createElement('label')
        label.classList.add('in_table');
